...
import { AppBar, Toolbar, Typography, Avatar, Menu, MenuItem, IconButton, Box, Badge, Paper, Link } from '@mui/material';
      ...
      <Toolbar style={{ minHeight: '60px', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Link href="/dashboard" passHref>
          <a style={{ textDecoration: 'none' }}>
            <Paper variant="">
              <img src="emporiumlogo.png" height={40} width={200}  
              alt="Emporium Logo" />
            </Paper>
          </a>
        </Link>
        ...
        <Box display="flex" alignItems="center">
          ...
          <IconButton edge="end" onClick={handleMenu} color="inherit" style={{ marginLeft: '16px', borderRadius:4 }}>
            <Avatar alt="User Photo" src={userData?.user?.userImage ? userData.user.userImage : "/image/user.png"} />
            <Typography variant="body1" style={{ marginLeft: '10px' }}>
              {userData?.user?.email || 'Guest'} 
            </Typography>
          </IconButton>
          <Menu className="mt-[50px]"
            anchorEl={anchorEl}
            ...
            PaperProps={{
              style: {
                marginTop: '50px', // Adds 60px space below the anchor element
              }
            }}
          >
